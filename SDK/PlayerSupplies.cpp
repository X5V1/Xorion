...
void PlayerSupplies::ensureIntegration() const {
    if (!suppliesPtr ||
        ...
        logF("[PlayerSupplies] Missing integration. Ensure InitSDK() succeeded for %s.", kMinecraftVersion.c_str());
...
void PlayerSupplies::setSelectedHotbarSlot(int slot) {
    ...
    if (!PlayerSupplies_setSelectedSlot_fn) {
        logF("[PlayerSupplies] setSelectedHotbarSlot signature missing for %s", kMinecraftVersion.c_str());
        return;
    }

...
static const MC_Color selectedModuleColor = moduleColor.lerp(whiteColor, 0.08f);
...
void ClickGui::renderBanner() {
    ...
    if (banner != nullptr) {
        // Base size for 1080p; scales lightly with screen height
        Vec2 winSize = Game.getClientInstance()->getGuiData()->windowSize;
        float scale = fmax(0.85f, fmin(1.25f, winSize.y / 1080.0f));
        Vec2 bannerSize = Vec2(300.0f * scale, 100.0f * scale);

        // Center horizontally, small margin from top
        float topMargin = 20.0f * scale;
        Vec2 pos = Vec2((winSize.x - bannerSize.x) / 2.0f, topMargin);
...
void ClickGui::renderCategory(Category category) {
                        ...
                        for (auto setting : *settings) {
                            ...
                            case ValueType::ENUM_T: {
                                ...
                                if (setting->minValue->_bool) {
                                    ...
                                    for (auto it = enumData->Entrys.begin();
                                        ...
                                        bool isEven = e % 2 == 0;
                                        Vec4 rectPos2;
                                        rectPos2.x = currentXOffset;
                                        rectPos2.y = currentYOffset;
                                        rectPos2.z = xEnd;
                                        rectPos2.w = currentYOffset + textHeight + (textPaddingY * 2);
                                        EnumEntry i = *it._Ptr;
                                        char name[0x21];
                                        ...
                                        if (name[0] != 0)
                                            name[0] = toupper(name[0]);
                                        std::string elTexto = name;
                                        windowSize->x = fmax(windowSize->x, DrawUtils::getTextWidth(&elTexto, textSize) + 5);
                                        Vec2 textPos2 = Vec2(currentXOffset + textPaddingX + 5, currentYOffset + textPaddingY);
                                        Vec4 selectableSurface = Vec4(
                                            textPos2.x,
                                            rectPos2.y,
                                            xEnd,
                                        ...
                                        DrawUtils::fillRectangle(rectPos2, ClientColors::clickGuiModuleColor, backgroundAlpha);
                                        DrawUtils::fillRectangle(selectableSurface, col, backgroundAlpha);
                                        DrawUtils::drawText(textPos2, &elTexto, whiteColor);
                            ...
                            case ValueType::FLOAT_T: {
                                // Text and background
                                {
                                    ...
                                    Vec4 rectPos2 = Vec4(currentXOffset, currentYOffset - textHeight - textPaddingY, xEnd, currentYOffset);
                                    DrawUtils::fillRectangle(rectPos2, ClientColors::clickGuiModuleColor, backgroundAlpha);
                                }
                                ...
                                // Slider
                                {
                                    ...
                                    Vec4 rectPos2 = Vec4(currentXOffset, currentYOffset, xEnd, currentYOffset + textHeight + (textPaddingY * 2));
                                    const bool areWeFocused = rect.contains(&mousePos);

                                    DrawUtils::fillRectangle(rectPos2, ClientColors::clickGuiModuleColor, backgroundAlpha);  // Background
                            ...
                            case ValueType::INT_T: {
                                ...
                                // Slider
                                {
                                    ...
                                    float value = (float)fmax(0, setting->value->_int - minValue);
                                    if (value > maxValue)
                                        value = maxValue;
                                    value /= maxValue;
                                    const float endXlol = (xEnd - textPaddingX) - (currentXOffset + textPaddingX + 5);
    ...
    // anti idiot
    {
        Vec2 windowSize2 = Game.getClientInstance()->getGuiData()->windowSize;

        if (ourWindow->pos.x + ourWindow->size.x > windowSize2.x) {
            ourWindow->pos.x = windowSize2.x - ourWindow->size.x;
        }

        if (ourWindow->pos.y + ourWindow->size.y > windowSize2.y) {
            ourWindow->pos.y = windowSize2.y - ourWindow->size.y;
        }
